...
// CloseAfterSummary writes the window summary and only then marks the window
// closed, so a failed summary leaves the window open for reprocessing.
func (svc *Service) CloseAfterSummary(windowID string) error {
	...
	if err := svc.summaries.Save(summary); err != nil {
		return err
	}
	return svc.windows.MarkClosed(w.ID)
}
